...
public class TenantInstitutionalEssentialsPublishedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(TenantInstitutionalEssentialsPublishedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theTenantInstitutionalEssentials.commitNeeded) {
				pageContext.getOut().print(theTenantInstitutionalEssentials.getPublishedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing TenantInstitutionalEssentials for publishedAt tag ");
	...
	public void setPublishedAt(Timestamp publishedAt) throws JspException {
		try {
			TenantInstitutionalEssentials theTenantInstitutionalEssentials = (TenantInstitutionalEssentials)findAncestorWithClass(this, TenantInstitutionalEssentials.class);
			theTenantInstitutionalEssentials.setPublishedAt(publishedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing TenantInstitutionalEssentials for publishedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
