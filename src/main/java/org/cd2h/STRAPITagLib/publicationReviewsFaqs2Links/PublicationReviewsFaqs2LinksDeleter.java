...
public class PublicationReviewsFaqs2LinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Logger log = LogManager.getLogger(PublicationReviewsFaqs2LinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.publication_reviews_faqs_2_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (faqId == 0 ? "" : " and faq_id = ? ")
            ...
            log.error("JDBC error generating PublicationReviewsFaqs2Links deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating PublicationReviewsFaqs2Links deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating PublicationReviewsFaqs2Links deleter",e);
			}
