...
public class TenantDurRequests extends STRAPITagLibTagSupport {

	static TenantDurRequests currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(TenantDurRequests.class);
	...
	private TenantDurRequests cachedTenantDurRequests = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			TenantDurRequestsIterator theTenantDurRequestsIterator = (TenantDurRequestsIterator)findAncestorWithClass(this, TenantDurRequestsIterator.class);

			if (theTenantDurRequestsIterator != null) {
				ID = theTenantDurRequestsIterator.getID();
			}

			if (theTenantDurRequestsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new TenantDurRequests and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a TenantDurRequests from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,request_header,request_block,overview_header,overview_block,process_header,prereq_header,prereq_block,submit_header,submit_block,submit_block_2,submit_block_3,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.tenant_dur_requests where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			TenantDurRequests currentTenantDurRequests = (TenantDurRequests) pageContext.getAttribute("tag_tenantDurRequests");
			if(currentTenantDurRequests != null){
				cachedTenantDurRequests = currentTenantDurRequests;
			}
			currentTenantDurRequests = this;
			pageContext.setAttribute((var == null ? "tag_tenantDurRequests" : var), currentTenantDurRequests);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedTenantDurRequests != null){
				pageContext.setAttribute((var == null ? "tag_tenantDurRequests" : var), this.cachedTenantDurRequests);
			}else{
				pageContext.removeAttribute((var == null ? "tag_tenantDurRequests" : var));
				this.cachedTenantDurRequests = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.tenant_dur_requests set header = ?, request_header = ?, request_block = ?, overview_header = ?, overview_block = ?, process_header = ?, prereq_header = ?, prereq_block = ?, submit_header = ?, submit_block = ?, submit_block_2 = ?, submit_block_3 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.tenant_dur_requests(header,request_header,request_block,overview_header,overview_block,process_header,prereq_header,prereq_block,submit_header,submit_block,submit_block_2,submit_block_3,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,requestHeader);
		stmt.setString(3,requestBlock);
		stmt.setString(4,overviewHeader);
