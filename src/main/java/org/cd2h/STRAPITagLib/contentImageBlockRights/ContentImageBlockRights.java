...
public class ContentImageBlockRights extends STRAPITagLibTagSupport {

	static ContentImageBlockRights currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(ContentImageBlockRights.class);
	...
	private ContentImageBlockRights cachedContentImageBlockRights = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ContentImageBlockRightsIterator theContentImageBlockRightsIterator = (ContentImageBlockRightsIterator)findAncestorWithClass(this, ContentImageBlockRightsIterator.class);

			if (theContentImageBlockRightsIterator != null) {
				ID = theContentImageBlockRightsIterator.getID();
			}

			if (theContentImageBlockRightsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new ContentImageBlockRights and to generate a new ID
		...
		if(pageContext != null){
			ContentImageBlockRights currentContentImageBlockRights = (ContentImageBlockRights) pageContext.getAttribute("tag_contentImageBlockRights");
			if(currentContentImageBlockRights != null){
				cachedContentImageBlockRights = currentContentImageBlockRights;
			}
			currentContentImageBlockRights = this;
			pageContext.setAttribute((var == null ? "tag_contentImageBlockRights" : var), currentContentImageBlockRights);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedContentImageBlockRights != null){
				pageContext.setAttribute((var == null ? "tag_contentImageBlockRights" : var), this.cachedContentImageBlockRights);
			}else{
				pageContext.removeAttribute((var == null ? "tag_contentImageBlockRights" : var));
				this.cachedContentImageBlockRights = null;
			}
		}
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new ContentImageBlockRights " + ID);
		}
