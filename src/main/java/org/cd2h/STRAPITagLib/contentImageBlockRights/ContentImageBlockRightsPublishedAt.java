...
public class ContentImageBlockRightsPublishedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(ContentImageBlockRightsPublishedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theContentImageBlockRights.commitNeeded) {
				pageContext.getOut().print(theContentImageBlockRights.getPublishedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing ContentImageBlockRights for publishedAt tag ");
	...
	public void setPublishedAt(Timestamp publishedAt) throws JspException {
		try {
			ContentImageBlockRights theContentImageBlockRights = (ContentImageBlockRights)findAncestorWithClass(this, ContentImageBlockRights.class);
			theContentImageBlockRights.setPublishedAt(publishedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing ContentImageBlockRights for publishedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
