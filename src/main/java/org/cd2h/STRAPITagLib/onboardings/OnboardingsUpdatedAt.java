...
public class OnboardingsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(OnboardingsUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theOnboardings.commitNeeded) {
				pageContext.getOut().print(theOnboardings.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing Onboardings for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			Onboardings theOnboardings = (Onboardings)findAncestorWithClass(this, Onboardings.class);
			theOnboardings.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing Onboardings for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
