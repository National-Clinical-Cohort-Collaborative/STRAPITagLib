...
public class MissionsPartnerGroupsLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int missionId = 0;
	...
	private static final Logger log = LogManager.getLogger(MissionsPartnerGroupsLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.missions_partner_groups_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating MissionsPartnerGroupsLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating MissionsPartnerGroupsLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating MissionsPartnerGroupsLinks deleter",e);
			}
