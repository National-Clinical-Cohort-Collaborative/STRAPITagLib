...
public class NewsItemsCreatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(NewsItemsCreatedAt.class);

	public int doStartTag() throws JspException {
		try {
			NewsItems theNewsItems = (NewsItems)findAncestorWithClass(this, NewsItems.class);
			if (!theNewsItems.commitNeeded) {
				pageContext.getOut().print(theNewsItems.getCreatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing NewsItems for createdAt tag ");
	...
	public void setCreatedAt(Timestamp createdAt) throws JspException {
		try {
			NewsItems theNewsItems = (NewsItems)findAncestorWithClass(this, NewsItems.class);
			theNewsItems.setCreatedAt(createdAt);
		} catch (Exception e) {
			log.error("Can't find enclosing NewsItems for createdAt tag ", e);
			freeConnection();
			Tag parent = getParent();
