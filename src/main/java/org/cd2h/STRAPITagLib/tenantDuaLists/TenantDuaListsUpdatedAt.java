...
public class TenantDuaListsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(TenantDuaListsUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theTenantDuaLists.commitNeeded) {
				pageContext.getOut().print(theTenantDuaLists.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing TenantDuaLists for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			TenantDuaLists theTenantDuaLists = (TenantDuaLists)findAncestorWithClass(this, TenantDuaLists.class);
			theTenantDuaLists.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing TenantDuaLists for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
