...
public class TenantDuaLists extends STRAPITagLibTagSupport {

	static TenantDuaLists currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(TenantDuaLists.class);
	...
	private TenantDuaLists cachedTenantDuaLists = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			TenantDuaListsIterator theTenantDuaListsIterator = (TenantDuaListsIterator)findAncestorWithClass(this, TenantDuaListsIterator.class);

			if (theTenantDuaListsIterator != null) {
				ID = theTenantDuaListsIterator.getID();
			}

			if (theTenantDuaListsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new TenantDuaLists and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a TenantDuaLists from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.tenant_dua_lists where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			TenantDuaLists currentTenantDuaLists = (TenantDuaLists) pageContext.getAttribute("tag_tenantDuaLists");
			if(currentTenantDuaLists != null){
				cachedTenantDuaLists = currentTenantDuaLists;
			}
			currentTenantDuaLists = this;
			pageContext.setAttribute((var == null ? "tag_tenantDuaLists" : var), currentTenantDuaLists);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedTenantDuaLists != null){
				pageContext.setAttribute((var == null ? "tag_tenantDuaLists" : var), this.cachedTenantDuaLists);
			}else{
				pageContext.removeAttribute((var == null ? "tag_tenantDuaLists" : var));
				this.cachedTenantDuaLists = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.tenant_dua_lists set header = ?, block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new TenantDuaLists " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.tenant_dua_lists(header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block1);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
