...
public class WorkstreamInstancesBiosLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Logger log = LogManager.getLogger(WorkstreamInstancesBiosLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.workstream_instances_bios_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (bioId == 0 ? "" : " and bio_id = ? ")
            ...
            log.error("JDBC error generating WorkstreamInstancesBiosLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating WorkstreamInstancesBiosLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating WorkstreamInstancesBiosLinks deleter",e);
			}
