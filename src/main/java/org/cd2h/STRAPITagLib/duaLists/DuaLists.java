...
public class DuaLists extends STRAPITagLibTagSupport {

	static DuaLists currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(DuaLists.class);
	...
	private DuaLists cachedDuaLists = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			DuaListsIterator theDuaListsIterator = (DuaListsIterator)findAncestorWithClass(this, DuaListsIterator.class);

			if (theDuaListsIterator != null) {
				ID = theDuaListsIterator.getID();
			}

			if (theDuaListsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new DuaLists and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a DuaLists from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.dua_lists where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			DuaLists currentDuaLists = (DuaLists) pageContext.getAttribute("tag_duaLists");
			if(currentDuaLists != null){
				cachedDuaLists = currentDuaLists;
			}
			currentDuaLists = this;
			pageContext.setAttribute((var == null ? "tag_duaLists" : var), currentDuaLists);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedDuaLists != null){
				pageContext.setAttribute((var == null ? "tag_duaLists" : var), this.cachedDuaLists);
			}else{
				pageContext.removeAttribute((var == null ? "tag_duaLists" : var));
				this.cachedDuaLists = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.dua_lists set header = ?, block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new DuaLists " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.dua_lists(header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block1);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
