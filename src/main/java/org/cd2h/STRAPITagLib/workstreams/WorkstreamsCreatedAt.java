...
public class WorkstreamsCreatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(WorkstreamsCreatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theWorkstreams.commitNeeded) {
				pageContext.getOut().print(theWorkstreams.getCreatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing Workstreams for createdAt tag ");
	...
	public void setCreatedAt(Timestamp createdAt) throws JspException {
		try {
			Workstreams theWorkstreams = (Workstreams)findAncestorWithClass(this, Workstreams.class);
			theWorkstreams.setCreatedAt(createdAt);
		} catch (Exception e) {
			log.error("Can't find enclosing Workstreams for createdAt tag ", e);
			freeConnection();
			Tag parent = getParent();
