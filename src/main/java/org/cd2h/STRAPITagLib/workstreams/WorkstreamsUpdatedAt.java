...
public class WorkstreamsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(WorkstreamsUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theWorkstreams.commitNeeded) {
				pageContext.getOut().print(theWorkstreams.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing Workstreams for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			Workstreams theWorkstreams = (Workstreams)findAncestorWithClass(this, Workstreams.class);
			theWorkstreams.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing Workstreams for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
