...
public class Tools extends STRAPITagLibTagSupport {

	static Tools currentInstance = null;
	...
	private Tools cachedTools = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ToolsIterator theToolsIterator = (ToolsIterator)findAncestorWithClass(this, ToolsIterator.class);

			if (theToolsIterator != null) {
				ID = theToolsIterator.getID();
			}

			if (theToolsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Tools and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Tools from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,header_2,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.tools where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Tools currentTools = (Tools) pageContext.getAttribute("tag_tools");
			if(currentTools != null){
				cachedTools = currentTools;
			}
			currentTools = this;
			pageContext.setAttribute((var == null ? "tag_tools" : var), currentTools);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedTools != null){
				pageContext.setAttribute((var == null ? "tag_tools" : var), this.cachedTools);
			}else{
				pageContext.removeAttribute((var == null ? "tag_tools" : var));
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.tools set header = ?, header_2 = ?, block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Tools " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.tools(header,header_2,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,header2);
		stmt.setString(3,block1);
		stmt.setTimestamp(4,createdAt);
