...
public class CovidExtensionsFileLinkagesLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Logger log = LogManager.getLogger(CovidExtensionsFileLinkagesLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.covid_extensions_file_linkages_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating CovidExtensionsFileLinkagesLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating CovidExtensionsFileLinkagesLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating CovidExtensionsFileLinkagesLinks deleter",e);
			}
