...
public class Redirects extends STRAPITagLibTagSupport {

	static Redirects currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Redirects.class);
	...
	private Redirects cachedRedirects = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			RedirectsIterator theRedirectsIterator = (RedirectsIterator)findAncestorWithClass(this, RedirectsIterator.class);

			if (theRedirectsIterator != null) {
				ID = theRedirectsIterator.getID();
			}

			if (theRedirectsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Redirects and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Redirects from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select source,target,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.redirects where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Redirects currentRedirects = (Redirects) pageContext.getAttribute("tag_redirects");
			if(currentRedirects != null){
				cachedRedirects = currentRedirects;
			}
			currentRedirects = this;
			pageContext.setAttribute((var == null ? "tag_redirects" : var), currentRedirects);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedRedirects != null){
				pageContext.setAttribute((var == null ? "tag_redirects" : var), this.cachedRedirects);
			}else{
				pageContext.removeAttribute((var == null ? "tag_redirects" : var));
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.redirects set source = ?, target = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Redirects " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.redirects(source,target,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,source);
		stmt.setString(2,target);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
	...
	public static String targetValue() throws JspException {
		try {
			return currentInstance.getTarget();
		} catch (Exception e) {
			 throw new JspTagException("Error in tag function targetValue()");
		}
	}
