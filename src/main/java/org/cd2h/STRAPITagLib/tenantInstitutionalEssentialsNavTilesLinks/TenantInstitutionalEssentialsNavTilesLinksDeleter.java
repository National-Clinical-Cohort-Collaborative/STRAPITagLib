...
public class TenantInstitutionalEssentialsNavTilesLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Logger log = LogManager.getLogger(TenantInstitutionalEssentialsNavTilesLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.tenant_institutional_essentials_nav_tiles_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating TenantInstitutionalEssentialsNavTilesLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating TenantInstitutionalEssentialsNavTilesLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating TenantInstitutionalEssentialsNavTilesLinks deleter",e);
			}
