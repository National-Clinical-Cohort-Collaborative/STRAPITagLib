...
public class PprlsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(PprlsUpdatedAt.class);

	public int doStartTag() throws JspException {
		try {
			Pprls thePprls = (Pprls)findAncestorWithClass(this, Pprls.class);
			if (!thePprls.commitNeeded) {
				pageContext.getOut().print(thePprls.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing Pprls for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			Pprls thePprls = (Pprls)findAncestorWithClass(this, Pprls.class);
			thePprls.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing Pprls for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
