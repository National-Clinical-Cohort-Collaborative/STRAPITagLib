...
public class PprlsDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    String header = null;
	...
	private static final Logger log = LogManager.getLogger(PprlsDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.pprls where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? "));
            ...
            log.error("JDBC error generating Pprls deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating Pprls deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating Pprls deleter",e);
			}
