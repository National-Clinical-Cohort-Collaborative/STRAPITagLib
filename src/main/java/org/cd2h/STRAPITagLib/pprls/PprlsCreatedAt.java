...
public class PprlsCreatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(PprlsCreatedAt.class);

	public int doStartTag() throws JspException {
		try {
			Pprls thePprls = (Pprls)findAncestorWithClass(this, Pprls.class);
			if (!thePprls.commitNeeded) {
				pageContext.getOut().print(thePprls.getCreatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing Pprls for createdAt tag ");
	...
	public void setCreatedAt(Timestamp createdAt) throws JspException {
		try {
			Pprls thePprls = (Pprls)findAncestorWithClass(this, Pprls.class);
			thePprls.setCreatedAt(createdAt);
		} catch (Exception e) {
			log.error("Can't find enclosing Pprls for createdAt tag ", e);
			freeConnection();
			Tag parent = getParent();
