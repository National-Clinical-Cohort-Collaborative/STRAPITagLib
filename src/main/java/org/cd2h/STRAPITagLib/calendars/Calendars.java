...
public class Calendars extends STRAPITagLibTagSupport {

	static Calendars currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Calendars.class);
	...
	private Calendars cachedCalendars = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			CalendarsIterator theCalendarsIterator = (CalendarsIterator)findAncestorWithClass(this, CalendarsIterator.class);

			if (theCalendarsIterator != null) {
				ID = theCalendarsIterator.getID();
			}

			if (theCalendarsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Calendars and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Calendars from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.calendars where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Calendars currentCalendars = (Calendars) pageContext.getAttribute("tag_calendars");
			if(currentCalendars != null){
				cachedCalendars = currentCalendars;
			}
			currentCalendars = this;
			pageContext.setAttribute((var == null ? "tag_calendars" : var), currentCalendars);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedCalendars != null){
				pageContext.setAttribute((var == null ? "tag_calendars" : var), this.cachedCalendars);
			}else{
				pageContext.removeAttribute((var == null ? "tag_calendars" : var));
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.calendars set header = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Calendars " + ID);
		}

		if (header == null){
			header = "";
		}
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.calendars(header,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setTimestamp(2,createdAt);
		stmt.setTimestamp(3,updatedAt);
		stmt.setTimestamp(4,publishedAt);
