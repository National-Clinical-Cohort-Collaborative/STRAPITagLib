...
public class BiosBioGroupsLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int bioId = 0;
    int bioGroupId = 0;
	...
	private static final Logger log = LogManager.getLogger(BiosBioGroupsLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.bios_bio_groups_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating BiosBioGroupsLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating BiosBioGroupsLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating BiosBioGroupsLinks deleter",e);
			}
