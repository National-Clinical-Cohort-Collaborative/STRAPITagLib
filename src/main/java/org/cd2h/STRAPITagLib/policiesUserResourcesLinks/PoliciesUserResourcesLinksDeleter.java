...
public class PoliciesUserResourcesLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int policyId = 0;
    int resourceId = 0;
	...
	private static final Logger log = LogManager.getLogger(PoliciesUserResourcesLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.policies_user_resources_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating PoliciesUserResourcesLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating PoliciesUserResourcesLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating PoliciesUserResourcesLinks deleter",e);
			}
