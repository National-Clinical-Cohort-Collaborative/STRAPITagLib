...
public class DomainTeamsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(DomainTeamsUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theDomainTeams.commitNeeded) {
				pageContext.getOut().print(theDomainTeams.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing DomainTeams for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			DomainTeams theDomainTeams = (DomainTeams)findAncestorWithClass(this, DomainTeams.class);
			theDomainTeams.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing DomainTeams for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
