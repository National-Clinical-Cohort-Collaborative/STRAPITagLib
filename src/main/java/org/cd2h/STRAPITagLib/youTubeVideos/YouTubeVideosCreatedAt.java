...
public class YouTubeVideosCreatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(YouTubeVideosCreatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theYouTubeVideos.commitNeeded) {
				pageContext.getOut().print(theYouTubeVideos.getCreatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing YouTubeVideos for createdAt tag ");
	...
	public void setCreatedAt(Timestamp createdAt) throws JspException {
		try {
			YouTubeVideos theYouTubeVideos = (YouTubeVideos)findAncestorWithClass(this, YouTubeVideos.class);
			theYouTubeVideos.setCreatedAt(createdAt);
		} catch (Exception e) {
			log.error("Can't find enclosing YouTubeVideos for createdAt tag ", e);
			freeConnection();
			Tag parent = getParent();
