...
public class Liaisons extends STRAPITagLibTagSupport {

	static Liaisons currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Liaisons.class);
	...
	private Liaisons cachedLiaisons = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			LiaisonsIterator theLiaisonsIterator = (LiaisonsIterator)findAncestorWithClass(this, LiaisonsIterator.class);

			if (theLiaisonsIterator != null) {
				ID = theLiaisonsIterator.getID();
			}

			if (theLiaisonsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Liaisons and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Liaisons from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.liaisons where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Liaisons currentLiaisons = (Liaisons) pageContext.getAttribute("tag_liaisons");
			if(currentLiaisons != null){
				cachedLiaisons = currentLiaisons;
			}
			currentLiaisons = this;
			pageContext.setAttribute((var == null ? "tag_liaisons" : var), currentLiaisons);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedLiaisons != null){
				pageContext.setAttribute((var == null ? "tag_liaisons" : var), this.cachedLiaisons);
			}else{
				pageContext.removeAttribute((var == null ? "tag_liaisons" : var));
				this.cachedLiaisons = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.liaisons set header = ?, block = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Liaisons " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.liaisons(header,block,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
