...
public class EnclaveEssentialsDescriptionLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int enclaveEssentialId = 0;
    int contentImageBlockLeftId = 0;
	Vector<STRAPITagLibTagSupport> parentEntities = new Vector<STRAPITagLibTagSupport>();

	private static final Logger log = LogManager.getLogger(EnclaveEssentialsDescriptionLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.enclave_essentials_description_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating EnclaveEssentialsDescriptionLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating EnclaveEssentialsDescriptionLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating EnclaveEssentialsDescriptionLinks deleter",e);
			}
