...
public class Newsletters extends STRAPITagLibTagSupport {

	static Newsletters currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Newsletters.class);
	...
	private Newsletters cachedNewsletters = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			NewslettersIterator theNewslettersIterator = (NewslettersIterator)findAncestorWithClass(this, NewslettersIterator.class);

			if (theNewslettersIterator != null) {
				ID = theNewslettersIterator.getID();
			}

			if (theNewslettersIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Newsletters and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Newsletters from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.newsletters where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Newsletters currentNewsletters = (Newsletters) pageContext.getAttribute("tag_newsletters");
			if(currentNewsletters != null){
				cachedNewsletters = currentNewsletters;
			}
			currentNewsletters = this;
			pageContext.setAttribute((var == null ? "tag_newsletters" : var), currentNewsletters);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedNewsletters != null){
				pageContext.setAttribute((var == null ? "tag_newsletters" : var), this.cachedNewsletters);
			}else{
				pageContext.removeAttribute((var == null ? "tag_newsletters" : var));
				this.cachedNewsletters = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.newsletters set header = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Newsletters " + ID);
		}

		if (header == null){
			header = "";
		}
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.newsletters(header,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setTimestamp(2,createdAt);
		stmt.setTimestamp(3,updatedAt);
		stmt.setTimestamp(4,publishedAt);
