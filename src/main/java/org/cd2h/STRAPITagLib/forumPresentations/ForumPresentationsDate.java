...
public class ForumPresentationsDate extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(ForumPresentationsDate.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theForumPresentations.commitNeeded) {
				pageContext.getOut().print(theForumPresentations.getDate());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing ForumPresentations for date tag ");
	...
	public void setDate(Timestamp date) throws JspException {
		try {
			ForumPresentations theForumPresentations = (ForumPresentations)findAncestorWithClass(this, ForumPresentations.class);
			theForumPresentations.setDate(date);
		} catch (Exception e) {
			log.error("Can't find enclosing ForumPresentations for date tag ", e);
			freeConnection();
			Tag parent = getParent();
