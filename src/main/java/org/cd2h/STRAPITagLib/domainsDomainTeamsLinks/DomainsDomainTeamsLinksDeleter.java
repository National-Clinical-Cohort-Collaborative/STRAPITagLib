...
public class DomainsDomainTeamsLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int domainId = 0;
    int domainTeamId = 0;
	...
	private static final Logger log = LogManager.getLogger(DomainsDomainTeamsLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.domains_domain_teams_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating DomainsDomainTeamsLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating DomainsDomainTeamsLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating DomainsDomainTeamsLinks deleter",e);
			}
