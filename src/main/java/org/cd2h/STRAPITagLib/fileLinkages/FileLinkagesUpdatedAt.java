...
public class FileLinkagesUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(FileLinkagesUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theFileLinkages.commitNeeded) {
				pageContext.getOut().print(theFileLinkages.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing FileLinkages for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			FileLinkages theFileLinkages = (FileLinkages)findAncestorWithClass(this, FileLinkages.class);
			theFileLinkages.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing FileLinkages for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
