...
public class FileLinkages extends STRAPITagLibTagSupport {

	static FileLinkages currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(FileLinkages.class);
	...
	private FileLinkages cachedFileLinkages = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			FileLinkagesIterator theFileLinkagesIterator = (FileLinkagesIterator)findAncestorWithClass(this, FileLinkagesIterator.class);

			if (theFileLinkagesIterator != null) {
				ID = theFileLinkagesIterator.getID();
			}

			if (theFileLinkagesIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new FileLinkages and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a FileLinkages from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select label,description,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.file_linkages where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			FileLinkages currentFileLinkages = (FileLinkages) pageContext.getAttribute("tag_fileLinkages");
			if(currentFileLinkages != null){
				cachedFileLinkages = currentFileLinkages;
			}
			currentFileLinkages = this;
			pageContext.setAttribute((var == null ? "tag_fileLinkages" : var), currentFileLinkages);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedFileLinkages != null){
				pageContext.setAttribute((var == null ? "tag_fileLinkages" : var), this.cachedFileLinkages);
			}else{
				pageContext.removeAttribute((var == null ? "tag_fileLinkages" : var));
				this.cachedFileLinkages = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.file_linkages set label = ?, description = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new FileLinkages " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.file_linkages(label,description,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,label);
		stmt.setString(2,description);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
