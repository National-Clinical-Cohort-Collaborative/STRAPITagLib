...
public class NewsletterIssues extends STRAPITagLibTagSupport {

	static NewsletterIssues currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(NewsletterIssues.class);
	...
	private NewsletterIssues cachedNewsletterIssues = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			NewsletterIssuesIterator theNewsletterIssuesIterator = (NewsletterIssuesIterator)findAncestorWithClass(this, NewsletterIssuesIterator.class);

			if (theNewsletterIssuesIterator != null) {
				ID = theNewsletterIssuesIterator.getID();
			}

			if (theNewsletterIssuesIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new NewsletterIssues and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a NewsletterIssues from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select issue,url,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.newsletter_issues where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			NewsletterIssues currentNewsletterIssues = (NewsletterIssues) pageContext.getAttribute("tag_newsletterIssues");
			if(currentNewsletterIssues != null){
				cachedNewsletterIssues = currentNewsletterIssues;
			}
			currentNewsletterIssues = this;
			pageContext.setAttribute((var == null ? "tag_newsletterIssues" : var), currentNewsletterIssues);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedNewsletterIssues != null){
				pageContext.setAttribute((var == null ? "tag_newsletterIssues" : var), this.cachedNewsletterIssues);
			}else{
				pageContext.removeAttribute((var == null ? "tag_newsletterIssues" : var));
				this.cachedNewsletterIssues = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.newsletter_issues set issue = ?, url = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new NewsletterIssues " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.newsletter_issues(issue,url,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,issue);
		stmt.setString(2,url);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
