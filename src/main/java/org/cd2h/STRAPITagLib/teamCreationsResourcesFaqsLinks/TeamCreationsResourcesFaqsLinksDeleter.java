...
public class TeamCreationsResourcesFaqsLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Logger log = LogManager.getLogger(TeamCreationsResourcesFaqsLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.team_creations_resources_faqs_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
                                                        + (faqId == 0 ? "" : " and faq_id = ? ")
            ...
            log.error("JDBC error generating TeamCreationsResourcesFaqsLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating TeamCreationsResourcesFaqsLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating TeamCreationsResourcesFaqsLinks deleter",e);
			}
