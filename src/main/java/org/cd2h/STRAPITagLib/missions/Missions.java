...
public class Missions extends STRAPITagLibTagSupport {

	static Missions currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Missions.class);
	...
	private Missions cachedMissions = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			MissionsIterator theMissionsIterator = (MissionsIterator)findAncestorWithClass(this, MissionsIterator.class);

			if (theMissionsIterator != null) {
				ID = theMissionsIterator.getID();
			}

			if (theMissionsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Missions and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Missions from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select block_1,block_2,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.missions where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Missions currentMissions = (Missions) pageContext.getAttribute("tag_missions");
			if(currentMissions != null){
				cachedMissions = currentMissions;
			}
			currentMissions = this;
			pageContext.setAttribute((var == null ? "tag_missions" : var), currentMissions);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedMissions != null){
				pageContext.setAttribute((var == null ? "tag_missions" : var), this.cachedMissions);
			}else{
				pageContext.removeAttribute((var == null ? "tag_missions" : var));
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.missions set block_1 = ?, block_2 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Missions " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.missions(block_1,block_2,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,block1);
		stmt.setString(2,block2);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
