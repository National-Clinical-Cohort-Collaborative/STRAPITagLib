...
public class InstitutionalEssentials extends STRAPITagLibTagSupport {

	static InstitutionalEssentials currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(InstitutionalEssentials.class);
	...
	private InstitutionalEssentials cachedInstitutionalEssentials = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			InstitutionalEssentialsIterator theInstitutionalEssentialsIterator = (InstitutionalEssentialsIterator)findAncestorWithClass(this, InstitutionalEssentialsIterator.class);

			if (theInstitutionalEssentialsIterator != null) {
				ID = theInstitutionalEssentialsIterator.getID();
			}

			if (theInstitutionalEssentialsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new InstitutionalEssentials and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a InstitutionalEssentials from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,header_2,block,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.institutional_essentials where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			InstitutionalEssentials currentInstitutionalEssentials = (InstitutionalEssentials) pageContext.getAttribute("tag_institutionalEssentials");
			if(currentInstitutionalEssentials != null){
				cachedInstitutionalEssentials = currentInstitutionalEssentials;
			}
			currentInstitutionalEssentials = this;
			pageContext.setAttribute((var == null ? "tag_institutionalEssentials" : var), currentInstitutionalEssentials);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedInstitutionalEssentials != null){
				pageContext.setAttribute((var == null ? "tag_institutionalEssentials" : var), this.cachedInstitutionalEssentials);
			}else{
				pageContext.removeAttribute((var == null ? "tag_institutionalEssentials" : var));
				this.cachedInstitutionalEssentials = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.institutional_essentials set header = ?, header_2 = ?, block = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new InstitutionalEssentials " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.institutional_essentials(header,header_2,block,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,header2);
		stmt.setString(3,block);
		stmt.setTimestamp(4,createdAt);
