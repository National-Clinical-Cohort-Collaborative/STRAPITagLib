...
public class ExternalDatasets extends STRAPITagLibTagSupport {

	static ExternalDatasets currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(ExternalDatasets.class);
	...
	private ExternalDatasets cachedExternalDatasets = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ExternalDatasetsIterator theExternalDatasetsIterator = (ExternalDatasetsIterator)findAncestorWithClass(this, ExternalDatasetsIterator.class);

			if (theExternalDatasetsIterator != null) {
				ID = theExternalDatasetsIterator.getID();
			}

			if (theExternalDatasetsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new ExternalDatasets and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a ExternalDatasets from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.external_datasets where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			ExternalDatasets currentExternalDatasets = (ExternalDatasets) pageContext.getAttribute("tag_externalDatasets");
			if(currentExternalDatasets != null){
				cachedExternalDatasets = currentExternalDatasets;
			}
			currentExternalDatasets = this;
			pageContext.setAttribute((var == null ? "tag_externalDatasets" : var), currentExternalDatasets);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedExternalDatasets != null){
				pageContext.setAttribute((var == null ? "tag_externalDatasets" : var), this.cachedExternalDatasets);
			}else{
				pageContext.removeAttribute((var == null ? "tag_externalDatasets" : var));
				this.cachedExternalDatasets = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.external_datasets set header = ?, block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new ExternalDatasets " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.external_datasets(header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block1);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
