...
public class ContentImageBlockLefts extends STRAPITagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theContentImageBlockLeftsIterator == null && ID == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select content,header,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.content_image_block_lefts where id = ?");
				stmt.setInt(1,ID);
	...
	public int doEndTag() throws JspException {
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.content_image_block_lefts set content = ?, header = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.content_image_block_lefts(content,header,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,content);
		stmt.setString(2,header);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
