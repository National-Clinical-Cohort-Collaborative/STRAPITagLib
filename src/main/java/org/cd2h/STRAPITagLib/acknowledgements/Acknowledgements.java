...
public class Acknowledgements extends STRAPITagLibTagSupport {

	static Acknowledgements currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Acknowledgements.class);
	...
	private Acknowledgements cachedAcknowledgements = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			AcknowledgementsIterator theAcknowledgementsIterator = (AcknowledgementsIterator)findAncestorWithClass(this, AcknowledgementsIterator.class);

			if (theAcknowledgementsIterator != null) {
				ID = theAcknowledgementsIterator.getID();
			}

			if (theAcknowledgementsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Acknowledgements and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Acknowledgements from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.acknowledgements where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Acknowledgements currentAcknowledgements = (Acknowledgements) pageContext.getAttribute("tag_acknowledgements");
			if(currentAcknowledgements != null){
				cachedAcknowledgements = currentAcknowledgements;
			}
			currentAcknowledgements = this;
			pageContext.setAttribute((var == null ? "tag_acknowledgements" : var), currentAcknowledgements);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedAcknowledgements != null){
				pageContext.setAttribute((var == null ? "tag_acknowledgements" : var), this.cachedAcknowledgements);
			}else{
				pageContext.removeAttribute((var == null ? "tag_acknowledgements" : var));
				this.cachedAcknowledgements = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.acknowledgements set header = ?, block = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Acknowledgements " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.acknowledgements(header,block,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
