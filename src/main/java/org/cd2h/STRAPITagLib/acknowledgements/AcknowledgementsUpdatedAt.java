...
public class AcknowledgementsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(AcknowledgementsUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theAcknowledgements.commitNeeded) {
				pageContext.getOut().print(theAcknowledgements.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing Acknowledgements for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			Acknowledgements theAcknowledgements = (Acknowledgements)findAncestorWithClass(this, Acknowledgements.class);
			theAcknowledgements.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing Acknowledgements for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
