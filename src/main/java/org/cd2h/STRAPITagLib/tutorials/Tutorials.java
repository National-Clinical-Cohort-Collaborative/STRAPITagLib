...
public class Tutorials extends STRAPITagLibTagSupport {

	static Tutorials currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Tutorials.class);
	...
	private Tutorials cachedTutorials = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			TutorialsIterator theTutorialsIterator = (TutorialsIterator)findAncestorWithClass(this, TutorialsIterator.class);

			if (theTutorialsIterator != null) {
				ID = theTutorialsIterator.getID();
			}

			if (theTutorialsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Tutorials and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Tutorials from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.tutorials where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Tutorials currentTutorials = (Tutorials) pageContext.getAttribute("tag_tutorials");
			if(currentTutorials != null){
				cachedTutorials = currentTutorials;
			}
			currentTutorials = this;
			pageContext.setAttribute((var == null ? "tag_tutorials" : var), currentTutorials);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedTutorials != null){
				pageContext.setAttribute((var == null ? "tag_tutorials" : var), this.cachedTutorials);
			}else{
				pageContext.removeAttribute((var == null ? "tag_tutorials" : var));
				this.cachedTutorials = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.tutorials set header = ?, block = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Tutorials " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.tutorials(header,block,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
