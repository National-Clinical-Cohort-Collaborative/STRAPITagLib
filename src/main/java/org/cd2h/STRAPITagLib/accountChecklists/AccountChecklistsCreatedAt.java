...
public class AccountChecklistsCreatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(AccountChecklistsCreatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theAccountChecklists.commitNeeded) {
				pageContext.getOut().print(theAccountChecklists.getCreatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing AccountChecklists for createdAt tag ");
	...
	public void setCreatedAt(Timestamp createdAt) throws JspException {
		try {
			AccountChecklists theAccountChecklists = (AccountChecklists)findAncestorWithClass(this, AccountChecklists.class);
			theAccountChecklists.setCreatedAt(createdAt);
		} catch (Exception e) {
			log.error("Can't find enclosing AccountChecklists for createdAt tag ", e);
			freeConnection();
			Tag parent = getParent();
