...
public class OnboardingsNavTileLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int onboardingId = 0;
	...
	private static final Logger log = LogManager.getLogger(OnboardingsNavTileLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.onboardings_nav_tile_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating OnboardingsNavTileLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating OnboardingsNavTileLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating OnboardingsNavTileLinks deleter",e);
			}
