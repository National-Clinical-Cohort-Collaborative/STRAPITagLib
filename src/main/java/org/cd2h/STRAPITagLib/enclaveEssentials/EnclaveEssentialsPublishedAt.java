...
public class EnclaveEssentialsPublishedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(EnclaveEssentialsPublishedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!theEnclaveEssentials.commitNeeded) {
				pageContext.getOut().print(theEnclaveEssentials.getPublishedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing EnclaveEssentials for publishedAt tag ");
	...
	public void setPublishedAt(Timestamp publishedAt) throws JspException {
		try {
			EnclaveEssentials theEnclaveEssentials = (EnclaveEssentials)findAncestorWithClass(this, EnclaveEssentials.class);
			theEnclaveEssentials.setPublishedAt(publishedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing EnclaveEssentials for publishedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
