...
public class Projects extends STRAPITagLibTagSupport {

	static Projects currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Projects.class);
	...
	private Projects cachedProjects = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ProjectsIterator theProjectsIterator = (ProjectsIterator)findAncestorWithClass(this, ProjectsIterator.class);

			if (theProjectsIterator != null) {
				ID = theProjectsIterator.getID();
			}

			if (theProjectsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Projects and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Projects from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.projects where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Projects currentProjects = (Projects) pageContext.getAttribute("tag_projects");
			if(currentProjects != null){
				cachedProjects = currentProjects;
			}
			currentProjects = this;
			pageContext.setAttribute((var == null ? "tag_projects" : var), currentProjects);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedProjects != null){
				pageContext.setAttribute((var == null ? "tag_projects" : var), this.cachedProjects);
			}else{
				pageContext.removeAttribute((var == null ? "tag_projects" : var));
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.projects set header = ?, block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Projects " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.projects(header,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block1);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
