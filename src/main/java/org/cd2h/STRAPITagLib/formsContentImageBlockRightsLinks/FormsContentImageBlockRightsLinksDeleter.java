...
public class FormsContentImageBlockRightsLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int formId = 0;
	...
	private static final Logger log = LogManager.getLogger(FormsContentImageBlockRightsLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.forms_content_image_block_rights_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating FormsContentImageBlockRightsLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating FormsContentImageBlockRightsLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating FormsContentImageBlockRightsLinks deleter",e);
			}
