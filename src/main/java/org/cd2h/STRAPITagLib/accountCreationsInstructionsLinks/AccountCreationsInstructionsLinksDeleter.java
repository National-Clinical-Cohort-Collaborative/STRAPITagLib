...
public class AccountCreationsInstructionsLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
	...
	private static final Logger log = LogManager.getLogger(AccountCreationsInstructionsLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.account_creations_instructions_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating AccountCreationsInstructionsLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating AccountCreationsInstructionsLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating AccountCreationsInstructionsLinks deleter",e);
			}
