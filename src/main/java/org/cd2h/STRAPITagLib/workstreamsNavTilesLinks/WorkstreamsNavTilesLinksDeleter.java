...
public class WorkstreamsNavTilesLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int workstreamId = 0;
    int navTileId = 0;
	...
	private static final Logger log = LogManager.getLogger(WorkstreamsNavTilesLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.workstreams_nav_tiles_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating WorkstreamsNavTilesLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating WorkstreamsNavTilesLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating WorkstreamsNavTilesLinks deleter",e);
			}
