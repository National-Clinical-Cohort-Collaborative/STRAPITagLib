...
public class Supports extends STRAPITagLibTagSupport {

	static Supports currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Supports.class);
	...
	private Supports cachedSupports = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			SupportsIterator theSupportsIterator = (SupportsIterator)findAncestorWithClass(this, SupportsIterator.class);

			if (theSupportsIterator != null) {
				ID = theSupportsIterator.getID();
			}

			if (theSupportsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Supports and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Supports from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,block,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.supports where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Supports currentSupports = (Supports) pageContext.getAttribute("tag_supports");
			if(currentSupports != null){
				cachedSupports = currentSupports;
			}
			currentSupports = this;
			pageContext.setAttribute((var == null ? "tag_supports" : var), currentSupports);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedSupports != null){
				pageContext.setAttribute((var == null ? "tag_supports" : var), this.cachedSupports);
			}else{
				pageContext.removeAttribute((var == null ? "tag_supports" : var));
				this.cachedSupports = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.supports set header = ?, block = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Supports " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.supports(header,block,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,block);
		stmt.setTimestamp(3,createdAt);
		stmt.setTimestamp(4,updatedAt);
