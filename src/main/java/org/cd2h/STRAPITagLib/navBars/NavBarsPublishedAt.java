...
public class NavBarsPublishedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(NavBarsPublishedAt.class);

	public int doStartTag() throws JspException {
		try {
			NavBars theNavBars = (NavBars)findAncestorWithClass(this, NavBars.class);
			if (!theNavBars.commitNeeded) {
				pageContext.getOut().print(theNavBars.getPublishedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing NavBars for publishedAt tag ");
	...
	public void setPublishedAt(Timestamp publishedAt) throws JspException {
		try {
			NavBars theNavBars = (NavBars)findAncestorWithClass(this, NavBars.class);
			theNavBars.setPublishedAt(publishedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing NavBars for publishedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
