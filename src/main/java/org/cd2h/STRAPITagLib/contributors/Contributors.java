...
public class Contributors extends STRAPITagLibTagSupport {

	static Contributors currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Contributors.class);
	...
	private Contributors cachedContributors = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ContributorsIterator theContributorsIterator = (ContributorsIterator)findAncestorWithClass(this, ContributorsIterator.class);

			if (theContributorsIterator != null) {
				ID = theContributorsIterator.getID();
			}

			if (theContributorsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Contributors and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Contributors from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.contributors where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Contributors currentContributors = (Contributors) pageContext.getAttribute("tag_contributors");
			if(currentContributors != null){
				cachedContributors = currentContributors;
			}
			currentContributors = this;
			pageContext.setAttribute((var == null ? "tag_contributors" : var), currentContributors);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedContributors != null){
				pageContext.setAttribute((var == null ? "tag_contributors" : var), this.cachedContributors);
			}else{
				pageContext.removeAttribute((var == null ? "tag_contributors" : var));
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.contributors set block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Contributors " + ID);
		}

		if (block1 == null){
			block1 = "";
		}
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.contributors(block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,block1);
		stmt.setTimestamp(2,createdAt);
		stmt.setTimestamp(3,updatedAt);
		stmt.setTimestamp(4,publishedAt);
