...
public class Forums extends STRAPITagLibTagSupport {

	static Forums currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Logger log = LogManager.getLogger(Forums.class);
	...
	private Forums cachedForums = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ForumsIterator theForumsIterator = (ForumsIterator)findAncestorWithClass(this, ForumsIterator.class);

			if (theForumsIterator != null) {
				ID = theForumsIterator.getID();
			}

			if (theForumsIterator == null && ID == 0) {
				// no ID was provided - the default is to assume that it is a new Forums and to generate a new ID
				ID = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or ID was provided as an attribute - we need to load a Forums from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select header,header_2,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id from strapi.forums where id = ?");
				stmt.setInt(1,ID);
		...
		if(pageContext != null){
			Forums currentForums = (Forums) pageContext.getAttribute("tag_forums");
			if(currentForums != null){
				cachedForums = currentForums;
			}
			currentForums = this;
			pageContext.setAttribute((var == null ? "tag_forums" : var), currentForums);
		}
	...
	public int doEndTag() throws JspException {
		...
		if(pageContext != null){
			if(this.cachedForums != null){
				pageContext.setAttribute((var == null ? "tag_forums" : var), this.cachedForums);
			}else{
				pageContext.removeAttribute((var == null ? "tag_forums" : var));
				this.cachedForums = null;
			}
		}
		...
		try {
			...
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update strapi.forums set header = ?, header_2 = ?, block_1 = ?, created_at = ?, updated_at = ?, published_at = ?, created_by_id = ?, updated_by_id = ? where id = ? ");
				stmt.setString( 1, header );
				stmt.setString( 2, header2 );
				stmt.setString( 3, block1 );
	...
	public void insertEntity() throws JspException, SQLException {
		if (ID == 0) {
			ID = Sequence.generateID();
			log.debug("generating new Forums " + ID);
		}
		...
		PreparedStatement stmt = getConnection().prepareStatement("insert into strapi.forums(header,header_2,block_1,created_at,updated_at,published_at,created_by_id,updated_by_id) values (?,?,?,?,?,?,?,?)", java.sql.Statement.RETURN_GENERATED_KEYS);
		stmt.setString(1,header);
		stmt.setString(2,header2);
		stmt.setString(3,block1);
		stmt.setTimestamp(4,createdAt);
