...
public class PublicHealthQuestionsUpdatedAt extends STRAPITagLibTagSupport {

	private static final Logger log = LogManager.getLogger(PublicHealthQuestionsUpdatedAt.class);
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!thePublicHealthQuestions.commitNeeded) {
				pageContext.getOut().print(thePublicHealthQuestions.getUpdatedAt());
			}
		} catch (Exception e) {
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: Can't find enclosing PublicHealthQuestions for updatedAt tag ");
	...
	public void setUpdatedAt(Timestamp updatedAt) throws JspException {
		try {
			PublicHealthQuestions thePublicHealthQuestions = (PublicHealthQuestions)findAncestorWithClass(this, PublicHealthQuestions.class);
			thePublicHealthQuestions.setUpdatedAt(updatedAt);
		} catch (Exception e) {
			log.error("Can't find enclosing PublicHealthQuestions for updatedAt tag ", e);
			freeConnection();
			Tag parent = getParent();
