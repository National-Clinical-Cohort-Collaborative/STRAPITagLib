...
public class WorkstreamsYouTubeVideoLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int workstreamId = 0;
    int youTubeVideoId = 0;
	Vector<STRAPITagLibTagSupport> parentEntities = new Vector<STRAPITagLibTagSupport>();

	private static final Logger log = LogManager.getLogger(WorkstreamsYouTubeVideoLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.workstreams_you_tube_video_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating WorkstreamsYouTubeVideoLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating WorkstreamsYouTubeVideoLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating WorkstreamsYouTubeVideoLinks deleter",e);
			}
