...
public class TrainingsEugYouTubeVideosLinksDeleter extends STRAPITagLibBodyTagSupport {
    int ID = 0;
    int trainingId = 0;
	...
	private static final Logger log = LogManager.getLogger(TrainingsEugYouTubeVideosLinksDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from strapi.trainings_eug_you_tube_videos_links where 1=1"
                                                        + (ID == 0 ? "" : " and id = ? ")
            ...
            log.error("JDBC error generating TrainingsEugYouTubeVideosLinks deleter", e);
			...
			if(parent != null){
				...
				pageContext.setAttribute("tagErrorMessage", "Error: JDBC error generating TrainingsEugYouTubeVideosLinks deleter");
				return parent.doEndTag();
			}else{
				throw new JspException("Error: JDBC error generating TrainingsEugYouTubeVideosLinks deleter",e);
			}
